...
#
# Tags look like `v<Misskey version>-<release>`, which is what this repository
# has always published (v2025.9.0-0 ... v2026.6.0-1):
#
# - if defaults/main.yml points at a Misskey version that has never been
#   released, the release counter restarts at 0 (`v2026.7.0-0`)
# - otherwise the counter is incremented (`v2026.7.0-1`), but only if something
...
# The commit-message approach this replaced could only ever mint a `-0`, so a
# fix to the role that did not come with a version bump was never released at
# all; and it once mistook Renovate's "Docker tag to v2026" subject - Misskey
# publishes a floating `2026` tag alongside the real ones - for a version, and
# published the `v2026-0` tag that this repository still carries.
...
# Anchored on `misskey_version:` so that neither a commented-out example nor
# `misskey_container_image_tag`, which is derived from it, can be mistaken for it.
version="$(sed -nE 's|^misskey_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Misskey version from $defaults_path"
	exit 1
fi

# Misskey's own version is carried without a leading `v` (the tags add one), but
